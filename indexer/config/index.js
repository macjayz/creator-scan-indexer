...
// Helper: Convert to lowercase for consistency
...
export const config = {
  ...
  factories: {
    zora: {
      address: normalizeAddress(process.env.ZORA_FACTORY_ADDRESS),
      startBlock: parseInt(process.env.START_BLOCK || '39246500'),
      abi: [
        'event CoinCreated(address indexed token, string name, string symbol, address indexed payoutRecipient)'
      ]
    },
    clanker: {
      address: normalizeAddress(process.env.CLANKER_FACTORY_ADDRESS),
      startBlock: parseInt(process.env.START_BLOCK || '39246500'),
      abi: [
